...
$null = "{null}";
if ($stmt = $dbcon->prepare("INSERT INTO session(ID,username,ldate,title,session,doctor) VALUES (?,?,?,?,?,?)")) 
	{
	$stmt->bind_param('ssssss',$null ,$user, $timeStamp, $title, $session, $currentU);
	$stmt->execute();
